...
static int run_detection_pipeline_nvdec(const PipelineConfig *config);
...
static int run_detection_pipeline_nvdec(const PipelineConfig *config) {
    ...
    if (video_hw_reader_open /* module: video/video_hw_reader */ (&reader, config->input_path, config->decoder_threads) != 0) {
    ...
    for (;;) {
        ...
        const double timestamp_ms = frame.timestamp_ms > 0.0 ? frame.timestamp_ms : (info->fps > 0.0 ? (double)frame_id * 1000.0 / info->fps : 0.0);
        for (size_t i = 0; i < detection_result.count; ++i) {
            detection_result.items[i].timestamp_ms = timestamp_ms;
        }
        ...
        if (config->draw_boxes) {
            if (cuda_overlay_draw_nv12_boxes /* module: gpu/cuda_overlay */ (&frame,
                                                                              ...
                                                                              frame.cuda_stream,
                                                                              &timing.overlay_ms) != 0) {
                log_error /* module: utils/logger */ ("CUDA overlay failed on frame %d: %s", frame_id, cuda_overlay_last_error /* module: gpu/cuda_overlay */ ());
                video_hw_reader_release_frame /* module: video/video_hw_reader */ (&reader, &frame);
                goto cleanup;
            }
            if (video_hw_writer_write_cuda_nv12 /* module: video/video_hw_writer */ (&writer, &frame, &timing) != 0) {
                log_error /* module: utils/logger */ ("NVENC encode failed on frame %d: %s", frame_id, video_hw_writer_last_error /* module: video/video_hw_writer */ ());
                video_hw_reader_release_frame /* module: video/video_hw_reader */ (&reader, &frame);
                goto cleanup;
            }
        }

        timing.process_ms = timing.preprocess_ms + timing.inference_ms + timing.download_ms + timing.postprocess_ms + timing.overlay_ms;
    ...
    if (writer_opened && video_hw_writer_flush /* module: video/video_hw_writer */ (&writer) != 0) {
        log_error /* module: utils/logger */ ("hardware writer flush failed: %s", video_hw_writer_last_error /* module: video/video_hw_writer */ ());
        goto cleanup;
    }
...
cleanup:
    if (cuda_nv12_frame_is_valid /* module: gpu/cuda_frame */ (&frame)) {
        video_hw_reader_release_frame /* module: video/video_hw_reader */ (&reader, &frame);
    }
    if (benchmark_opened) {
        benchmark_close_csv /* module: benchmark/benchmark */ (&benchmark);
...
static int run_detection_pipeline(const PipelineConfig *config) {
    ...
    if (config->decoder_mode == VIDEO_DECODER_NVDEC) {
        const int result = run_detection_pipeline_nvdec /* module: pipeline/pipeline_runner */ (config);
        if (result == 0 || config->decoder_fallback != DECODER_FALLBACK_CPU) {
            return result;
        }
        log_warn /* module: utils/logger */ ("NVDEC detection path failed; falling back to CPU-decoded NV12 detection");
    }
