...
static __global__ void nv12_to_nchw_fp32_kernel(const uint8_t *y_plane,
                                                const uint8_t *uv_plane,
                                                int src_width,
                                                int src_height,
                                                size_t y_stride,
    ...
    output[plane_size + index] = g / 255.0f;
    output[plane_size * 2 + index] = b / 255.0f;
}
...
cudaError_t vcp_cuda_preprocess_nv12_to_nchw_fp32(const uint8_t *d_y,
    ...
    const dim3 grid((unsigned int)((dst_width + block.x - 1) / block.x),
                    (unsigned int)((dst_height + block.y - 1) / block.y));
    nv12_to_nchw_fp32_kernel<<<grid, block, 0, stream>>>(d_y,
                                                         d_uv,
                                                         src_width,
                                                         src_height,
                                                         src_y_stride,
